...
class UpdatePFAViewController : UIViewController {
    ...
    override func viewDidLoad() {
        ...
        getPFAVideos(type: type) { contents in
    ...
    @objc func deleteContentClicked(){
        ...
        alert.addAction(UIAlertAction(title: "Delete", style: .destructive, handler: { action in
            ...
            let imageStorage = Storage.storage().reference().child(self.type!).child("\(self.contentModel!.id!).png")
            ...
            for multiVideoModel in self.multiVideoModels {
                ...
                var  docRef = Firestore.firestore().collection("PFA").document(self.type!)
              
              
                 docRef = docRef.collection("Videos").document(multiVideoModel.id!)
            ...
            let  docRef1 = Firestore.firestore().collection("PFA").document(self.type!)
            
          
          
            docRef1.delete { error in
                self.ProgressHUDHide()
                if let error = error {
...
extension UpdatePFAViewController :  UITextFieldDelegate, UITextViewDelegate {
            ...
            self.deleteFileFromS3(bucketName: "adfreadybucket", s3FileName: gest.path) { error in
                DispatchQueue.main.async {
                    ...
                    if let error = error {
                        ...
                        Firestore.firestore().collection("PFA").document(self.type!).collection("Videos").document(gest.id).delete()
...
extension UpdatePFAViewController :  UINavigationControllerDelegate, UIImagePickerControllerDelegate{
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
            ...
            if let videoURL = info[UIImagePickerController.InfoKey(rawValue: "UIImagePickerControllerMediaURL") ] as? URL {
                ...
                let videoID = Firestore.firestore().collection("PFA").document(self.type!).collection("Videos").document().documentID
...
extension UpdatePFAViewController : UIDocumentPickerDelegate {
...
extension UpdatePFAViewController : UITableViewDelegate, UITableViewDataSource {
