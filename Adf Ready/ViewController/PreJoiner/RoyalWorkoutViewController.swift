...
class RoyalWorkoutViewController : UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "workoutSubCatSeg" {
            ...
            if let VC = segue.destination as? RoyalWorkoutSubcatViewController {
                if let category = sender as? CategoryModel {
                    VC.catId = category.id
...
extension  RoyalWorkoutViewController : UITableViewDelegate, UITableViewDataSource {
